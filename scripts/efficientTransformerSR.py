...
class EfficientTransformerSR:
        ...
        with GradSelection():
            for x, y in batchLoader:
                ...
                batchLoader.set_description(
                    desc=f"{action} [{epochProgress}] -lglr {'%.1f' % (-math.log(self.learningRate, 10))} 🕐loss {'%.2f' % (loss.item() / len(y))} ➗loss {'%.2f' % (totalLoss / totalLen)}")
    ...
    def getModelName(self):
        return f"ETSR-lr{self.configs['startLearningRate']}-flip{self.configs['randomFlip']}-psize" \
